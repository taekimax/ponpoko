...
- 작성일: 2026-07-16
- 기획 상태: `done`
- 구현 진입 상태: W0/W1 `done`, W2 `partial`, W3 `blocked`
- 대상 게임: 보글보글(Bubble Bobble, `bublbobl`)
- 대상 클라이언트: iPhone 16 Pro Max 및 iPhone 15 Pro의 Safari tab/Home Screen web app (Android 제외)
...
flowchart LR
    ...
    SIG -->|"단기 credential"| TURN["Cloudflare Realtime TURN"]
...
| Cloudflare Realtime TURN | 직접 연결 실패 시 media/data relay | 장기 provider credential을 프런트엔드에 포함 |
...
W0는 정확한 origin을 `https://ponpoko-2p.taekimax.workers.dev`, WSS를 `wss://ponpoko-2p.taekimax.workers.dev/v1/session`으로 고정한다. Cloudflare Workers Free와 room별 SQLite-backed Durable Object/WebSocket Hibernation을 사용한다. 응답은 `Cache-Control: no-store`로 제공한다. 이 선택은 배포 완료를 뜻하지 않으며 실제 account/subdomain/secret 검증은 W4 진입 게이트다.
...
GitHub Pages에는 프런트엔드만 둔다. 별도 Cloudflare Workers/Durable Objects HTTPS/WSS 시그널링 서비스와 Cloudflare Realtime TURN을 사용한다. W0 선택만으로 외부 서비스, 비용, DNS, secret을 생성하거나 변경하지 않으며 실제 운영은 W4의 별도 review gate를 통과해야 한다.
...
- Cloudflare TURN key ID/API token이나 고정 TURN 계정을 JavaScript에 넣지 않는다.
- signaling이 Cloudflare Realtime API로 host/guest별 5분 TTL credential을 발급하고, 유효 세션에만 필요 시 갱신하며 종료·leave·만료에서 revoke한다.
...
- TURN credential revoke가 요청되더라도 연결 종료까지 짧은 지연이 있을 수 있으므로 5분 TTL과 즉시 revoke를 함께 사용하고 잔여 노출을 PoC risk로 기록한다.
- 브라우저에는 port 53 ICE URL을 주지 않는다. Realtime 월간 egress 800GB에서 credential 발급을 fail-closed하고, 유료 Workers 또는 무료 allowance 초과 과금은 승인하지 않는다.
...
| W4 | P3, P8, P9, P12 | Cloudflare Workers/Durable Objects signaling API와 Realtime TURN 단기 credential | 배포 가능한 server contract | expiry/rate-limit/origin/revoke/cost-cutoff/relay integration |
...
W1을 먼저 완료한 뒤 W2의 로컬 capture harness/adapter를 진행한다. W2의 실제 iPhone capture spike는 W3 이후 작업보다 먼저 닫는다. W4는 선택된 외부 서비스의 실제 account/domain/secret review가 필요하다. W5~W10은 각각 선행 계약을 통과한 뒤 작은 feature slice로 구현하며, scope와 무관한 리팩터링은 포함하지 않는다.
...
- D013-D017의 signaling/TURN/cost/ownership, QR dependency, 두-iPhone target matrix 기록
- 독립 Evaluator의 W0 contract diff 승인
- 실제 Cloudflare account/secret과 iPhone 두 대의 확보·검증은 각각 W4와 W2/W11의 별도 게이트로 유지
...
- 2026-07-19 direct-LAN HTTP same-page Safari 진단은 두 대상 모델에서 received preview가 검정이어서 실패했고 첫 false-ready 수정도 Phone A에서 검정을 정직하게 `failed`로 보고했다. 이후 WebGL display-buffer video bridge가 Phone A에서 fallback 없이, D020이 Phone B 대체 기기로 인정한 iPhone 15 Pro Max에서 fallback 1회 후 움직이는 수신 영상, `ready=true`, same-device game sound, prompt/error 0회를 보였다. 이로써 두 기기의 실용적 개별 media-path 호환성은 확인됐지만 receiver-only audio, secure origin/Home Screen, cross-device는 미검증이므로 Gate 1A 증거가 아니다. iPhone XS Max/iOS 17.4.1은 임시 호환성 정보일 뿐 D019를 대체하지 않는다.
...
현재 지원 pair는 Phone A iPhone 16 Pro Max와 Phone B iPhone 15 Pro이며 둘 다 iOS 26.5.2다. 2026-07-19 실용적 호환성 진단에 한해 build 미확인을 허용했다. 정식 Gate 1A/W11에서는 Settings build를 기록하고 Safari/Home Screen 및 host/guest 역할을 교대한다.
...
| TURN 비용·대역폭 | 중/상 | relay 비율/byte 증가 | 30fps·1.5Mbps, 5분 TTL, 세션 60분, 월 800GB credential cutoff; 무료 allowance/가격 변경 시 fail-closed |
...
| signaling 배포 provider·도메인 | `done` | Cloudflare Workers Free + SQLite Durable Objects, `ponpoko-2p.taekimax.workers.dev`; 실제 provisioning은 W4 review gate |
| TURN provider·비용·owner | `done` | Cloudflare Realtime TURN, USD 0, 800GB cutoff, no custom TURN DNS, user-owned Worker secrets |
| QR 생성 dependency | `done` | W7에서만 exact-pin `uqr@0.1.3`; in-app scanner 없음 |
| 실제 iPhone 테스트 기기·OS 조합 | `done` | D019: iPhone 16 Pro Max + iPhone 15 Pro, iOS 26.5.2, 역할 교대; build 미확인은 2026-07-19 진단에만 허용하고 정식 gate에서 Settings readback |

현재 구현 상태는 W0/W1 `done`, W2 `partial`, W3 `blocked`다. D021의 임시 W2 전용 same-Wi-Fi LAN HTTP A→B harness는 구현·독립 수락됐고, 실제 iPhone 실행에서 moving guest video, initial audible audio, zero prompt, guest CTA 1회, host fallback 1회, `ready=yes`를 확인했다. D022의 natural-silence 분류를 적용한 후속 물리 실행은 조용한 host 대기 화면을 Phone B에서 `수신 출력 무음`/`ready=no`로 잘못 분류했다. D023은 raw positive `totalAudioEnergy` delta 대신 W3C interval RMS `sqrt(Δenergy/Δduration)`를 사용하고, float receiver analyser와 보수적인 byte fallback, missing/reset-stat unknown 처리를 추가했다. Node 25.8.1에서 focused 2 files/55 tests, full 24 files/208 tests, typecheck, production build, static smoke, two-page LAN desktop/mobile smoke, query-off Bubble desktop/mobile, full flag-off browser/catalog/preparation-failure smoke, `git diff --check`가 통과했고 독립 Evaluator가 finding 없이 ACCEPT했다. D023 이후의 실제 iPhone 실행은 아직 없다. Node 24는 없어 CI parity를 주장하지 않는다. LAN HTTP는 secure-origin/Home Screen 증거가 아니며, D021의 role-swap waiver는 W2에만 적용되고 W11의 D019 exact pair·양 orientation·direct+TURN matrix는 바뀌지 않는다.
...
2. D013-D017의 signaling/TURN/cost/ownership, QR dependency, device matrix를 독립 평가한다.
3. 평가 통과 뒤 W1의 player-aware 입력을 완료하고, 그 다음 W2 capture adapter를 별도 slice로 구현한다.
